...
export const connectPrivateKeyWalletClick = () => {
  const privateKey = <HTMLInputElement>(
  ...
  connect
    ...
    .catch((error) => {
      if ("message" in error) {
        ...
        setConnectionStatus(
          "An error occurred trying to connect wallet: " + error
        );
      }
    });
};
...
export const updateContractState = () => {
  ...
  const refreshLoader = <HTMLInputElement>(
    document.querySelector("#refresh-loader")
  );
  ...
  CLIENT.getContractData<RawZkContractData>(address).then((contract) => {
    if (contract != null) {
      ...
      if (getEngineKeys() === undefined) {
        const engineKeys = contract.serializedContract.engines.engines.map(
          (e) =>
      ...
      const administrator = <HTMLElement>(
        document.querySelector("#administrator-value")
      );
      administrator.innerHTML = `${state.administrator.asString()}`;

      const noSalaries = <HTMLElement>document.querySelector("#num-salaries");
      noSalaries.innerHTML = `${countSalaries(
        contract.serializedContract.variables
      )}`;

      const averageSalaryResult = <HTMLElement>(
        document.querySelector("#average-result")
      );
      averageSalaryResult.innerHTML = `${state.averageSalaryResult ?? "None"}`;

      const numEmployees = <HTMLElement>(
        document.querySelector("#num-employees")
      );
      numEmployees.innerHTML = `${state.numEmployees ?? "None"}`;

      const contractState = <HTMLElement>(
        document.querySelector("#contract-state")
      );
...
export const updateInteractionVisibility = () => {
  const contractInteraction = <HTMLElement>(
    document.querySelector("#contract-interaction")
  );
